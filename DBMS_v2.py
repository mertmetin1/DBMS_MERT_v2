...
def export_tables():
    ...
    try:
        ...
        # CSV dosyasını oluşturmak için bir StringIO nesnesi kullanın
        csv_data = StringIO()
        csv_writer = csv.writer(csv_data)

        # Her tabloyu döngüye alın ve CSV dosyasına yazın
        for table in tables:
            table_name = table[list(table.keys())[0]]
            cursor.execute(f"SELECT * FROM {table_name}")
            rows = cursor.fetchall()

            # İlk olarak, tablo adını CSV dosyasına ekleyin
            csv_writer.writerow([f"Table: {table_name}"])

            # Sütun başlıklarını CSV dosyasına ekleyin
            cursor.description = [col[0] for col in cursor.description]
            csv_writer.writerow(cursor.description)

            # Satırları CSV dosyasına ekleyin
            for row in rows:
                csv_writer.writerow(row)

            # Tablo aralarında bir satır ekleyin
            csv_writer.writerow([])

        # CSV dosyasını bellekten alın ve kullanıcıya indirme olarak gönderin
        csv_data.seek(0)

        # CSV dosyasını kaydetmek için table_data.csv adında bir dosya oluşturun
        file_path = os.path.join(app.root_path, 'table_data.csv')
        with open(file_path, 'w') as file:
            file.write(csv_data.getvalue())

        return send_file(file_path, as_attachment=True, attachment_filename='table_data.csv', mimetype='text/csv')

    except Exception as e:
        print("Error exporting tables:", e)
        return Response("Error exporting tables", status=500)  # Internal server error
    finally:
